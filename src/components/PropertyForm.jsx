...
const baseSchema = z.object({
  ...
  commissionPercent: z.string().min(1, "Comissão obrigatória"),
  commissionValue: z.string().min(1, "Comissão obrigatória"),
...
const PropertyForm = ({ onSubmit }) => {
  ...
  useEffect(() => {
    if (price && commissionPercent) {
      ...
      setValue("commissionValue", calculatedValue); // Atualiza o campo de valor
    }
  ...
  return (
    <div>
      <FormProvider {...methods} onSubmit={handleSubmit(onSubmit)}>
        <form className="grid grid-cols-4 gap-x-8 gap-y-6 mb-8">
          <InputField
